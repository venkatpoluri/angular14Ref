...
@Component({
  selector: 'app-freight-period-grid',
  templateUrl: './freight-period-grid.component.html',
  styleUrls: ['./freight-period-grid.component.scss']
})
export class FreightPeriodGridComponent implements OnInit {
  ...
  constructor(private offcanvasService: NgbOffcanvas, private menuItemSerivce: MenuItemService, private route: ActivatedRoute,
    private toastr: ToastrService, private elementRef: ElementRef, private exportService: ExportService, private modalService: NgbModal, private currentUserService: CurrentUserService) {
  ...
  onSubmitPlateCostPeriod() {
    ...
    this.menuItemSerivce.updatePlateCostPeriod(this.menuItemId, this.platePeriodsForm.id, this.platePeriodsForm).subscribe({
      ...
      error: (err) => {
        this.toastr.error('Sorry Plate Cost Period Not Updated', 'Error')
        this.offcanvasService.dismiss('Cross click');
      },
    })
  }
